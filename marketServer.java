 ...
 public class marketServer {
	...
	private WineCatalog wc = WineCatalog.getCatalog();
	 ...
	 class ServerThread extends Thread {
				...
				try {
					...
					switch (command) {
						...
						case 'c':
							break;
						case 't':
							talkComm(inStream, outStream, u);
							break;
						case 'r':
							readComm(inStream, outStream, u);	
							break;
					}
				} catch (IOException e) {
					System.err.println(e.getMessage());
		...
		private void wallet(ObjectInputStream inStream, ObjectOutputStream outStream, User u) {
		...
		private synchronized void talkComm(ObjectInputStream inStream, ObjectOutputStream outStream, User u) {
			try {
				String toWhom = (String) inStream.readObject();
				// Cria o ficheiro que vai ficar com as mensagens do remetente
				File myObj = new File(toWhom+"Msg.txt");
				myObj.createNewFile();

				String[] msg = (String[]) inStream.readObject();
				// Escreve no ficheiro
				FileWriter myWriter = new FileWriter(toWhom+"Msg.txt", true);
		...
		private synchronized void readComm(ObjectInputStream inStream, ObjectOutputStream outStream, User u) {
			String name = u.getName();
			try {
				File myObj = new File(name+"Msg.txt");
